---
title: "Stump Grinding Hickory NC | Free Estimate"
h1: "Stump Grinding in Hickory, NC"
description: "Stump grinding in Hickory, NC runs $150 to $800 or more depending on stump size. $150 minimum callout. Licensed and insured. Free estimate."
intro: "[Tree Service Hickory NC](/) grinds stumps flush below grade for homeowners, landlords and small commercial properties across Catawba County. Every job starts with a look at the stump's diameter and root spread, then a written price before the grinder starts. The grinding itself takes the stump down below grade, then a cleanup leaves the mulch chips for you to keep or haul away. Single stumps, several from the same removal, or ones left behind by another crew, and we grind every one of them. We work on single-family lots, rental properties and small commercial sites. A mid-size grinder handles most yard jobs, and a larger machine comes out for wide, old-growth stumps or several in one visit. Licensed and insured on every job, with a free estimate before you pay for anything."
layer: main
hero: stump
inMenu: true
order: 4
faqs:
  - q: "How much does stump grinding cost in Hickory, NC?"
    a: "Our minimum callout is $150. Most jobs run $150 to $800 or more depending on the stump's diameter and how many stumps are on site. You get an exact price at the inspection."
  - q: "How deep do you grind a stump?"
    a: "Standard depth is 4 to 6 inches below grade, enough to cover with soil and grass. A job ahead of new construction or a driveway can go deeper on request."
  - q: "Will grinding remove the whole root system?"
    a: "No. Grinding removes the visible stump and the surface roots nearby. The deeper root system stays in the ground and breaks down naturally over time."
  - q: "Can you grind a stump the same day as a tree removal?"
    a: "Yes, that is available in the same visit as a removal, or on its own for a stump left by another crew."
  - q: "Is Tree Service Hickory NC licensed and insured?"
    a: "Yes, on every job."
  - q: "What happens to the wood chips?"
    a: "You can keep them for mulch or have the crew haul them off, whichever you prefer, at no extra charge."
---

Wet ground and sandy loam soil are common across Hickory yards, which is part of why a
stump left in place keeps sending up root suckers years later. See our
[Tree Removal](/tree-removal) page for the local climate detail behind that.

## Tree Services We Provide

### Tree Removal
Full removal of a dead, hazardous or unwanted tree. See our
[Tree Removal](/tree-removal) page for pricing and process.

### Tree Trimming
Structural pruning, canopy thinning and clearance work on a tree that is still sound.
See our [Tree Trimming](/tree-trimming) page for pricing and process.

### Emergency and Storm Damage Removal
A tree down on a structure or blocking a driveway after a storm. See our
[Emergency Storm Damage Tree Removal](/emergency-storm-damage-tree-removal) page.

### Hazardous Tree Removal
A dead or storm-weakened tree that has not fallen yet but poses a real risk. See our
[Hazardous Tree Removal](/hazardous-tree-removal) page.

### Land Clearing
Clearing a wooded lot for a build site or a driveway, priced per acre. See our
[Land Clearing](/land-clearing) page.

### Deadwood and Diseased Tree Removal
Dead limbs or a tree too far gone from disease to save. See our
[Deadwood and Diseased Tree Removal](/deadwood-diseased-tree-removal) page.

## Which Stump Grinding Job Do You Actually Need?

Three situations get called "stump grinding" and they price differently. **A single
recent stump** from a removal we just did grinds fastest, since there is no old root
flare to work around. **An old stump left by another crew**, often partly grown over
or with grass established around it, takes longer to locate the true edge of and grind
clean. **Multiple stumps on one property**, from a lot that has had several trees come
down over the years, price as a group visit rather than separate callouts. If you plan
to replant in the same spot, tell us before the grinder starts, since grinding depth
changes when new roots need room to establish.

## What Stump Grinding Costs in Hickory

Our minimum callout for stump grinding is **$150**. Most jobs run **$150 to $800 or
more**, driven mainly by the stump's diameter at ground level, with the number of
stumps and how easy the site is to access as secondary factors. A single small stump
under a couple feet across sits at the low end. A wide, old-growth stump, or several
stumps ground in the same visit, sits toward the top of that range or above it. You get
an exact number after we look at the stump, in writing, before the grinder starts.

## How Long Stump Grinding Takes

A single small stump usually grinds out in under an hour once the crew is set up. A
wide or old stump with an established root flare takes longer, since the grinder works
in from the edges rather than through the center in one pass. Multiple stumps on one
property add time per stump rather than a flat rate for the visit. You get a time
estimate when we look at the actual stump, not a generic range.

## How a Stump Grinding Job Runs, Start to Finish

Every stump grinding job follows the same four stages, whether it is one stump or a
whole lot.

### Inspection and a Written Price
We look at the stump's diameter, how deep the root flare sits, and what is nearby, a
fence, a walkway, a septic line, that the grinder needs to avoid. You get a price
before the machine starts.

### Protecting Your Property
The grinder throws chips and fine debris while it works, so anything close by, a
flower bed, a fence line, gets covered or moved first.

### The Grinding Itself
The stump gets ground down below grade, typically 4 to 6 inches, so the area can be
covered with soil and grass or replanted without a stump visible or in the way of a
mower.

### Cleanup and Chip Removal
Leftover stump chips get raked out and hauled off, or left in a pile for you to keep for mulch, at your choice. The area is left level and walkable.

## What Goes Wrong When Stump Grinding Is Done Badly

Grinding shallow instead of down to a proper depth leaves a stump that resurfaces as
the ground settles, or that a mower catches a year later. Skipping a check for nearby
utility lines or a septic line before grinding can turn a routine job into an expensive
repair. Leaving the root flare instead of a large main root uncut lets suckers keep
sending up new growth around the old stump for years, particularly with red maple.
Grinding without hauling or spreading the chips leaves a pile that has to be dealt with
separately, an extra trip that should have been part of the same visit.

## What We Grind To

Stumps get ground to 4 to 6 inches below grade as standard, deep enough to cover with
soil and reseed or lay sod without a bump showing. A job going in ahead of new
construction or a driveway gets ground deeper on request, since equipment weight and
grading tolerances are different from a lawn. Large surface roots radiating out from
the stump get ground back several feet where they would otherwise interfere with
mowing or a walkway.

## What We Do and What We Don't

We grind stumps of any size, single or multiple, fresh or old. We do not fill or
regrade the resulting hole beyond raking the chips level, that is landscaping work we
can point you toward. We do not remove the stump's root system entirely, that would
mean excavating the yard rather than grinding, and is rarely worth the cost for a
residential lot. If a stump sits directly over a septic line or a utility line, we say
so at the inspection rather than grinding blind.

## Why Choose Tree Service Hickory NC for Stump Grinding

- **Licensed and Insured**: every job is covered, on every property we work.
- **A Price Before the Grinder Starts**: the number you approve at inspection is the
  number on the invoice.
- **Right-Sized Equipment**: a mid-size grinder for yard stumps, a larger machine for
  wide or multiple stumps in one visit.
- **Chips Left or Hauled, Your Choice**: keep the mulch or have the crew take it, no
  extra charge either way.
- **Same-Visit With a Removal**: grind the stump the same day as a tree removal, no
  second trip or second crew.

## What a Typical Stump Grinding Job Looks Like

A homeowner calls about two stumps left in the side yard from trees removed years ago,
now surrounded by established grass. The inspection finds both stumps solid enough that
the grinder can work straight through rather than around soft, rotted wood. The crew grinds each stump down below grade, working the surface roots back a few feet from the main flare. The chips get raked into a pile at the edge of the yard for the homeowner to use as mulch. Total time on site runs under two hours for a job this size.

## Where We Work

A grinder and trailer travel the same route across Hickory, Conover and the rest of
Catawba County at no extra charge, one backyard stump or several on an older lot. See our [Service Areas](/service-areas)
page for the full list of towns we cover.

## Get Your Free Stump Grinding Estimate

Most Hickory stump grinding jobs run $150 to $800, and our $150 minimum covers the
smallest single-stump jobs. The price is in writing before the grinder starts, and
chips can be hauled or left for mulch, your choice. Call (828) 522-9430 to schedule an
estimate, or reach out through our contact page for a same-day callback.
